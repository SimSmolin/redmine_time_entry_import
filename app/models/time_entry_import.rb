...
class TimeEntryImport < Import
  ...
  def allowed_target_projects
    ...
    Project.allowed_to(user, :log_time).allowed_to(user, :import_time_entries).order(:lft)
  end
  ...
  def fixed_issue_id
    if mapping['fixed_issue_id'].to_s =~ /\Avalue:(\d+)\z/
      fixed_issue_id = $1.to_i
      allowed_target_issues.find_by_id(fixed_issue_id)
    end
  end

  def issue_fixed_check?
    mapping['issue_fixed_check'].to_s == "true"
  end
  ...
  def build_object(row, item)
    ...
    # attributes[:issue_id] = allowed_target_issues.named(issue_id).first.try(:id)

    if issue_fixed_check? && fixed_issue_id
      attributes[:issue_id] = fixed_issue_id.id
    else
      if issue_id = row_value(row, 'issue_id').presence
        attributes[:issue_id] = issue_id.to_i == 0 ? (issue_id.scan(/\d+/)[0]).to_s.to_i : issue_id.to_i
      else
